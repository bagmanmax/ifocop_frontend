...
document.getElementById("pause-btn").addEventListener("click", function () {
  ...
  console.log(pauseB);
  if (!pauseB) {
    animate();          // we use the animate fucction to return to the game once the boolean is false
  }
});
...
function testCrash() {
  for (let i = 0; i < pipesArray.length; i++) {
    if (
      (newPlayer.x < pipesArray[i].x + pipesArray[i].width &&
        newPlayer.x + newPlayer.width > pipesArray[i].x &&
        ((newPlayer.y < pipesArray[i].topPipe &&
          newPlayer.y + newPlayer.height > 0) ||
          (newPlayer.y > canvas.height - pipesArray[i].bottomPipe &&
            newPlayer.y + newPlayer.height < canvas.height))) ||
...
function showReward() {
  ...
  /* Declaration of all the images hidden I know its long i could have factorised but  i'm tired  */
  ...
  if (score == 3) {unlockImages1.style.display = "inline-block"; ringPoint.play();}
  if(score==5){unlockImages2.style.display="inline-block"; ringPoint.play();}
  if(score==7){unlockImages3.style.display="inline-block"; ringPoint.play();}
  if(score==9){unlockImages4.style.display="inline-block"; ringPoint.play();}
  if(score==11){unlockImages5.style.display="inline-block"; ringPoint.play();}
